...
@section('container')
  ...
  @if ($books->count())
    <div class="max-w-full mt-4 bg-gray-800 rounded-xl overflow-hidden shadow-md">
      @if ($books[0]->image)
        <img class="w-full max-h-[720px]" src="{{ asset('storage/' . $books[0]->image) }}" alt="">
      @else
      ...
      <div class="px-6 pb-2">
        ...
        <span class="capitalize inline-block bg-gray-200 rounded-full px-3 py-1 text-sm font-semibold text-gray-800 mr-2 mb-2">{{ $books[0]->created_at->diffForHumans() }}</span>
      </div>
    </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 {{ $books->lastPage() == $books->currentPage() && $books->count() == 1 ? '' : 'mt-4' }}">
        @foreach ($books->skip(1) as $book)
          <div class="max-w-full bg-gray-800 rounded-xl overflow-hidden shadow-md">
            @if ($book->image)
              <img class="w-full max-h-[720px]" src="{{ asset('storage/' . $book->image) }}" alt="">
            @else
            ...
            <div class="px-6 pb-2">
              ...
              <span class="capitalize inline-block bg-gray-200 rounded-full px-3 py-1 text-sm font-semibold text-gray-800 mr-2 mb-2">{{ $book->created_at->diffForHumans() }}</span>
            </div>
          </div>
        @endforeach
      </div>
